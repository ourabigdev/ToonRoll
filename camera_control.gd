...
func _update_rotation():
	var basis = Basis(Vector3(0.0, 1.0, 0.0), deg_to_rad(angle_y))
	basis *= Basis(Vector3(1.0, 0.0, 0.0), deg_to_rad(angle_x))
...
func _input(event):
	if event is InputEventMouseMotion:
		if Input.is_key_pressed(KEY_ALT):
			...
			var cam_origin = $Camera3D.transform.origin
			cam_origin.z = clamp(cam_origin.z + (event.relative.y * 0.1), 2.0, 1000.0)
			$Camera3D.transform.origin = cam_origin
